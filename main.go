...
func main() {
	...
	if *vercel {
		finalOutputDir = filepath.Join(".vercel", "output", "static", *outputDir)
	} else {
		// Default to SvelteKit structure
		finalOutputDir = filepath.Join(".svelte-kit", "output", "client", *outputDir)
	}

	// Remove existing directory if it exists - using rm -rf for speed
	if _, err := os.Stat(finalOutputDir); err == nil {
		cmd := exec.Command("rm", "-rf", finalOutputDir)
	...
	for i := 0; i < *numWorkers; i++ {
		wg.Add(1)
		go worker(&wg, wordChan, finalOutputDir, &processedCount)
	}
